              ...
              <div key={idx} className="relative">
                ...
                >
                  <div className="bg-white border border-fog-200 rounded-xl p-6 shadow-3d-sm hover:shadow-3d-md hover:border-runway-700/30 transition-all duration-300 transform-style-3d">
                    ...
                    {/* Image Display for Experience — Fixed portrait/aspect ratio layout */}
                    {item.type === 'experience' && (item as any).image && (
                      <div 
                        className="mt-4 rounded-lg overflow-hidden border border-fog-200 bg-paper-50/30 flex justify-center items-center p-3 shadow-inner"
                        style={{ transform: "translateZ(12px)" }}
                      >
                        <img 
                          src={(item as any).image} 
                          alt={item.type === 'education' ? item.degree : item.role}
                          className="max-w-full max-h-[420px] object-contain rounded-md shadow-sm hover:scale-[1.01] transition-transform duration-300"
                        />
                      </div>
                    )}
                    ...
                    {item.type === 'experience' && (item as any).subRole && (
                      <div 
                        className="bg-runway-700/5 rounded-lg p-4 mb-4 border border-runway-700/10 mt-4"
                        style={{ transform: "translateZ(12px)" }}
                      >
                        ...
                        {(item as any).subImage && (
                          <div 
                            className="mt-2 rounded-lg overflow-hidden border border-fog-200 bg-paper-50/30 flex justify-center items-center p-3 shadow-inner"
                          >
                            <img 
                              src={(item as any).subImage} 
                              alt={(item as any).subRole}
                              className="max-w-full max-h-[350px] object-contain rounded-md shadow-sm hover:scale-[1.01] transition-transform duration-300"
                            />
                          </div>
                        )}
                      </div>
